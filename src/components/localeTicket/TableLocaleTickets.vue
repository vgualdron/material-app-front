<template>
  <div>
    ...
    <form-locale-tickets
      ref="formLocaleTicketReference"
      :showNotificationsRef="showNotification"
      :listLocaleTicketsMountedRef="listLocaleTicketsMounted"
    />
  </div>
</template>
<script>
import { mapState, mapActions } from 'vuex';
import FormLocaleTickets from 'components/localeTicket/FormLocaleTickets.vue';
...
export default {
  ...
  computed: {
    ...
    validatedPermissions() {
      const statusCreate = havePermission('localeTicket.create');
      const statusEdit = havePermission('localeTicket.update');
      const statusDelete = havePermission('localeTicket.delete');
      return {
        create: {
  ...
  methods: {
    ...mapActions(localeTicketTypes.PATH, {
      listLocaleTickets: localeTicketTypes.actions.LIST_LOCALE_TICKETS,
      getLocaleTicket: localeTicketTypes.actions.GET_LOCALE_TICKET,
    }),
    async listLocaleTicketsMounted() {
      showLoading('Cargando Tiquetes ...', 'Por favor, espere', true);
      await this.listLocaleTickets();
      if (this.status === true) {
        this.data = this.localeTickets.map((element) => ({
          ...element,
          showEdit: true,
          showDelete: true,
        }));
    ...
    async showForm(id, type) {
      if (id === null) {
        this.$refs.formLocaleTicketReference.showModal(id, null, type);
      } else {
        await this.getLocaleTicket(id);
        if (this.status === true) {
          this.$refs.formLocaleTicketReference.showModal(id, { ...this.localeTicket }, type);
        } else {
